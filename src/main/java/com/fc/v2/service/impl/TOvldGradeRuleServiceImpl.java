...
import com.fc.v2.service.ITOvldGradeRuleService;

/**
 * 超限吨位等级判定规则 Service业务层处理（rule-eval 形状）
 *
 * @author fuce
 * @date 2026-09-14
 */
@Service
public class TOvldGradeRuleServiceImpl implements ITOvldGradeRuleService {

    /** 优先级降序；并列按 ruleCode 降序（确定性） */
    private static final Comparator<TOvldGradeRule> PRIORITY_THEN_CODE_DESC = new Comparator<TOvldGradeRule>() {
        @Override
        public int compare(TOvldGradeRule a, TOvldGradeRule b) {
    ...
    @Override
    public TOvldGradeRule selectTOvldGradeRuleById(Long id) {
        return this.ovldGradeRuleMapper.selectById(id);
    }

    @Override
    public List<TOvldGradeRule> listAvailable(Date at) {
        if (at == null) {
            return new java.util.ArrayList<TOvldGradeRule>();
        }
        return this.ovldGradeRuleMapper.selectList(new QueryWrapper<TOvldGradeRule>().eq("del_flag", 0));
    }
    ...
    public int evaluate(String ruleCode, BigDecimal input, Date at) {
        ...
        TOvldGradeRule rule = findRule(ruleCode, at);
    ...
    public int evaluateTop(BigDecimal input, Date at) {
        ...
        List<TOvldGradeRule> avail = listAvailable(at);
        if (avail.isEmpty()) {
            return 0;
        }
    ...
    public boolean usable(Long id, Date at) {
        ...
        TOvldGradeRule r = this.ovldGradeRuleMapper.selectById(id);
        return r != null;
    }
    ...
    public int countAvailable(Date at) {
        ...
        return this.ovldGradeRuleMapper
                .selectList(new QueryWrapper<TOvldGradeRule>().eq("del_flag", 0)).size();
    }

    private TOvldGradeRule findRule(String ruleCode, Date at) {
        List<TOvldGradeRule> hit = this.ovldGradeRuleMapper.selectList(new QueryWrapper<TOvldGradeRule>()
                .eq("del_flag", 0).eq("rule_code", ruleCode));
        TOvldGradeRule newest = null;
        for (TOvldGradeRule r : hit) {
    ...
    private BigDecimal resolveLimit(TOvldGradeRule r, int tier) {
        if (tier == 1) {
            return r.getTh1Max();
        }
        if (tier == 2) {
    ...
    /** 档案级默认阈值（明细未维护时的回退来源） */
    private BigDecimal defaultThreshold(TOvldGradeRule r, int tier) {
        return r.getTh1Max();
    }

    /** 分档：等于上限取高一档 */
    private int levelOf(TOvldGradeRule rule, BigDecimal input) {
